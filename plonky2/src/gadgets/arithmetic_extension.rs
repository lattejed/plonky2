...
impl<const D: usize> Default for QuotientGeneratorExtension<D> {
    fn default() -> Self {
        Self {
            numerator: ExtensionTarget([Target::VirtualTarget { index: 0 }; D]),
            denominator: ExtensionTarget([Target::VirtualTarget { index: 0 }; D]),
            quotient: ExtensionTarget([Target::VirtualTarget { index: 0 }; D]),
        }
    }
}
